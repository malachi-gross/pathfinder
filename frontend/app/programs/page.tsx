...
export default function ProgramsPage() {
  ...
  const { data: programs, isLoading } = useQuery({
    ...
    queryFn: () => programApi.getPrograms(filter === 'all' ? undefined : filter),
  });

  const filteredPrograms = programs || [];
  const majors = filteredPrograms.filter(p => p.program_type === 'major');
  const minors = filteredPrograms.filter(p => p.program_type === 'minor');
